...
Every bridge session, in-flight upstream setup task, expiry task, response task, and entered upstream response context is adapter-owned. Upstream setup is registered before its first cancellable network await. The setup-to-response ownership handoff is synchronized with session cleanup and runtime draining: once disconnect, expiry, or retirement has won that boundary, a late setup success is closed immediately and cannot publish a new response task or retain a runtime lease. Downstream disconnect, endpoint retirement, and application shutdown cancel/await adapter-owned work and close entered upstream contexts before releasing them. Retirement cancels bridge tasks before waiting for upstream-work leases, because setup or response work may itself own the leases being drained.
