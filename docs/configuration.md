...
Every bridge session, expiry task, response task, and entered upstream response context is adapter-owned. Downstream disconnect, endpoint retirement, and application shutdown cancel/await that work and close the upstream context before releasing it. Retirement cancels bridge tasks before waiting for upstream-work leases, because those tasks may themselves own the leases being drained.
