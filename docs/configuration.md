...
`restart.enabled` defaults to `false`. An unexpected process exit transitions the endpoint runtime to `FAILED`. A failed runtime rejects demand activation, so incoming request pressure cannot bypass backoff or reset the restart-attempt budget. When automatic restart is enabled, the supervisor alone advances recovery using bounded exponential backoff from `initial_backoff` through `max_backoff`, stopping after `max_attempts`. Restart-policy changes are authoritative in place: an already-sleeping backoff re-reads the currently published policy while holding the runtime lock before spawning, and aborts that pending restart if automatic restart has since been disabled or if its already-reserved attempt now exceeds a lowered `max_attempts` bound. An aborted or exhausted restart leaves the runtime `FAILED`; it does not reset the reserved attempt budget or route recovery through request-driven activation. Runtime state, active upstream leases, last exit code, and restart-attempt count are exposed by `/summary` for operational diagnosis.
