...
| Unbounded stream/session work | Runtime leases, bridge queue/session limits, TTL/backpressure policy, cancellation cleanup, and endpoint retirement bound work. Downstream stream disconnects are detected at the outer ASGI receive/send boundary so pre-2.4 `http.disconnect` and ASGI 2.4+ failed-send semantics both drive cancellation state and exactly-once stream-disconnect accounting. |
...
- In-process metrics reset on process restart; they are operational counters, not durable audit records. `stream_cancellations_total` is intentionally narrower than generic task cancellation and counts only confirmed downstream disconnects after response streaming has started.
