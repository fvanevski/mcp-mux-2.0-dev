...
- Adds payload-free structured request logs, request IDs, supported trace-context propagation, `/metrics`, and expanded `/summary` operational state; `summary` and `metrics` are reserved endpoint namespaces for those gateway-owned routes. Downstream stream cancellation is derived from actual ASGI disconnect/send-failure signals rather than task cancellation alone, and request correlation is preserved on framework-generated HTTP 500 responses.
