...
| `CUR-06` | An upstream that labels a response `application/json` but returns invalid UTF-8 or malformed JSON framing is contained at the gateway as HTTP 502 rather than repaired/replaced and forwarded as a successful MCP response; the legacy bridge likewise terminates invalid-UTF-8 JSON responses as an upstream failure. | Phase 6 release hardening | `test_negative_upstream_fixtures_fail_closed_and_are_observable[malformed-json]`, `[invalid-utf8-json]` + `test_invalid_utf8_json_upstream_fails_closed_in_legacy_bridge` |
