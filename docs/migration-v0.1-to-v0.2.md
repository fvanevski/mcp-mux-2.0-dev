...
11. **Malformed upstream JSON fails closed.** An upstream response labeled `application/json` must be valid UTF-8 and parse as JSON. Invalid UTF-8 is not repaired with replacement characters, and malformed JSON is no longer forwarded as a successful MCP payload; the canonical gateway returns its own HTTP 502 error and records the upstream failure, while the legacy bridge terminates the affected SSE session as an upstream-response failure.
